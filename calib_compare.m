...
for j = 1 : length(calibfiles)
    ...
    disp('Loads data')
    settings.dataPath =fullfile('..','MagnetometerArrayData_230601',datalists{2});
    load(settings.dataPath);
    
    settings.calibrationFilePath = fullfile('..','results', calibfiles{j});    
...
%% 
fp = fopen(fullfile('..','MAINS.txt'), 'w');

for i = 1 : length(datalists)
    settings.dataPath = fullfile('..','MagnetometerArrayData_230601',datalists{i});
    fprintf(fp, '%s\n', datalists{i});
    ...
    for j = 1 : length(calibfiles)
        ...
        settings.calibrationFilePath = fullfile('..','results', calibfiles{j});     
        load(settings.calibrationFilePath);
        disp('Using the calibration file');
        disp(calibfiles{j})
